...
Github地址：https://github.com/nimaltd/w25qxx
...
typedef struct
{
	...
	uint8_t		StatusRegister1;	// 状态寄存器1
	uint8_t		StatusRegister2;	// 状态寄存器2
	uint8_t		StatusRegister3;	// 状态寄存器3
	uint8_t		Lock;				// 锁定
...
#define W25QXX_DUMMY_BYTE         0xA5	// 无所谓的假数据
...
void W25qxx_WriteByte(uint8_t pBuffer, uint32_t WriteAddr_inBytes)
{
  ...
  W25qxx_Spi((WriteAddr_inBytes & 0xFF0000) >> 16); // 位操作的作用是从低位到高位一个字节一个字节的把WriteAddr_inBytes地址发送出去
